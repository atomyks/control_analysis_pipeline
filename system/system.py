...
class SystemLearning:
    ...
    def simulate(self, input_array: torch.tensor, initial_state: torch.tensor = None, true_state: torch.tensor = None,
        ...
        if input_array.dim() == 3:
            batch_size, time_length, num_inputs = input_array.shape
        ...
        if initial_state is not None:
            ...
            state = torch.zeros((batch_size, num_inputs))
        loss = 0
    ...
    # def simulate_subsystem(self, subsystem, input_, output_=None):
    #     loss = 0
    #     TIME_LEN, INPUT_DIM = input_.shape
    #     OUTPUT_DIM = subsystem.num_outputs
    #     STATE_DIM = subsystem.num_states
    #     states_predicted = torch.zeros((STATE_DIM,), dtype=torch.double)
    #     out_predicted = torch.zeros((TIME_LEN, OUTPUT_DIM), dtype=torch.double)
    #     for t in range(TIME_LEN):
    #         out, states_predicted = subsystem(states_predicted, input_[t])
    #         out_predicted[t] = out
    #         if output_ is not None:
    #             loss += self.loss_fn(out, output_[t])
    #     return out_predicted, loss

    # def learn_no_grad(self, subsystem):
    #     pass

    # def learn_grad(self, subsystem, inputs: list, outputs: list):
    #     epochs = 100
    #     NUM_SIGNALS = len(inputs)
    #     optimizer = optim.Adam(subsystem.parameters(), lr=0.01)
    #
    ...
    #             optimizer.zero_grad()
    #             _, loss = self.simulate_subsystem(subsystem, inputs[i], outputs[i])
    #             print(f"Loss: {loss}")
    #             loss.backward()
    #             optimizer.step()
